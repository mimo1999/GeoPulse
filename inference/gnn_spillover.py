...
# Structural edge priors: geographic contiguity (independent of model outputs).
_STRUCTURAL_EDGES_PATH = Path(__file__).parent.parent / "data" / "structural_edges.csv"
...
class GNNSpilloverEngine:
    ...
    def enrich(
        ...
        # 2. Load spillover adjacency — merge correlation edges with geographic priors.
        # Structural (contiguity) edges break the circularity where the GNN graph
        # is derived entirely from Transformer output correlations.
        spillover_rows = self._fetch_spillover()
        structural_rows = self._load_structural_edges()
        adj = build_adjacency(
            country_idx,
            spillover_rows + structural_rows,
    ...
    @staticmethod
    def _load_structural_edges() -> list[dict]:
        """Load geographic contiguity priors from CSV.

        These edges are independent of model outputs and prevent the adjacency
        graph from being purely circular (correlation of Transformer scores).
        Returns the same dict format as _fetch_spillover() for seamless merging.
        """
        if not _STRUCTURAL_EDGES_PATH.exists():
            logger.warning("Structural edges file not found: %s", _STRUCTURAL_EDGES_PATH)
            return []
        edges = []
        try:
            with open(_STRUCTURAL_EDGES_PATH, newline="") as f:
                for row in csv.DictReader(f):
                    try:
                        edges.append({
                            "country_a":       row["country_a"].strip(),
                            "country_b":       row["country_b"].strip(),
                            "spillover_weight": float(row["weight"]),
                        })
                    except (KeyError, ValueError):
                        continue
        except Exception as exc:
            logger.warning("Failed to load structural edges: %s", exc)
        logger.debug("Loaded %d structural (contiguity) edges", len(edges))
        return edges
