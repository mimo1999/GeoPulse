...
class BacktestMetrics:
    ...
    per_tier: dict = field(default_factory=dict)      # {risk_tier: {mae, rmse, n}}
    per_coverage: dict = field(default_factory=dict)  # {coverage_tier: {mae, rmse, n}}
...
def compute_per_coverage_metrics(
    actuals: np.ndarray,
    predictions: np.ndarray,
    coverage_tiers: np.ndarray,   # string array, e.g. ["high", "low", ...]
) -> dict:
    """Per-GDELT-coverage-tier metrics, exposing media-bias effect on accuracy."""
    tiers: dict[str, dict] = {}
    for a, p, t in zip(actuals, predictions, coverage_tiers):
