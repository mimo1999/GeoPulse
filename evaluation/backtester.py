...
class FoldPrediction:
    ...
    coverage_tier: str = "unknown"  # GDELT media coverage density at origin
...
class WalkForwardBacktester:
    ...
    def run(self) -> BacktestResults:
        ...
        for fold_idx, pivot_date in enumerate(test_dates):
            ...
            for country, steps in batch_steps:
                pivot_row   = feature_matrix[(country, pivot_date)]
                origin_risk = pivot_row["risk_score"]
                cov_tier    = pivot_row.get("coverage_tier") or "unknown"
                for step in steps:
    ...
    def _load_all_features(
        ...
        """
        from models.dataset import FEATURE_COLUMNS
        cols = ", ".join(FEATURE_COLUMNS + ["risk_score", "coverage_tier"])
    ...
    def _aggregate(
        ...
        if not all_actuals_arrs:
            ...
            # Per-coverage-tier metrics expose GDELT media bias in prediction accuracy.
            # Build arrays in fold_log order (not horizon-bucketed) so indices align.
            cov_preds = [p for p in fold_log if p.actual is not None]
            cov_a   = np.array([p.actual    for p in cov_preds])
            cov_p   = np.array([p.predicted for p in cov_preds])
            cov_t   = np.array([p.coverage_tier for p in cov_preds])
            per_coverage = compute_per_coverage_metrics(cov_a, cov_p, cov_t)
